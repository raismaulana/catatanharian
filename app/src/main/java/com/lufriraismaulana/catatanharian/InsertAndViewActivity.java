package com.lufriraismaulana.catatanharian;
...
public class InsertAndViewActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtFileName = findViewById(R.id.edt_filename);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_simpan:
                eventID = 2;
                if (!tempCatatan.equals(txtContent.getText().toString())) {
                    if (Build.VERSION.SDK_INT >= 23) {
                        if (periksaIzinPenyimpanan()) {
    ...
    void bacaFile() {
        ...
        File file = new File(path, txtFileName.getText().toString());
        if (file.exists()) {
    ...
    void buatDanUbah() {
        ...
        if (parent.exists()) {
            File file = new File(parent, txtFileName.getText().toString());
            FileOutputStream outputStream = null;
            try {
                file.createNewFile();
                outputStream = new FileOutputStream(file);
                OutputStreamWriter streamWriter = new OutputStreamWriter(outputStream);
                streamWriter.append(txtContent.getText());
                streamWriter.flush();
                streamWriter.close();
                streamWriter.flush();
                streamWriter.close();
            ...
            try {
                ...
                outputStream.write(txtContent.getText().toString().getBytes());
                outputStream.flush();
                outputStream.close();
            } catch (IOException e) {
                e.printStackTrace();
